...
data class Rates(
    ...
    companion object {
        var currenciesList: Map<String, Double> = mutableMapOf()
        fun convert(from: String, to: String, amount: Double): Double {
            val currencyRate = currenciesList[to]?.div(currenciesList[from] ?: 1.0)
            return currencyRate?.times(amount) ?: 0.0
        }
    }
    ...
    fun getCurrencies(): Map<String, Double> {
        ...
        this::class.members
            ...
            .forEach { property ->
                val value = property.getter.call(this) as? Double
                if (value != null) {
                    val serializedName = property.annotations
